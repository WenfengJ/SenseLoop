...
## 7.1 2026-09-25 复核与修复状态

本轮按 `senseloop-full-cycle-builder` 检查表复核后，Task14 中的早期问题已经部分修复：

已完成：

- 前端新增 `src/services/apiClient.ts`，核心页面已接后端 API。
- 今日 / 检测 / 问诊 / 报告 / 我的 底部导航已落地。
- 问诊页已接正式 Agent Chat API，消息、上下文、记忆和引用可落库。
- 体检报告 / 图片 / PDF 上传已接 `documents/summarize`，PDF 可提取文本，图片优先走视觉模型配置。
- 舌诊、饮食、排便、口气等入口已有真实点击反馈和保存状态。
- “我的”页已支持邮箱身份、游客身份、账号编号、设备编号、健康档案编号展示。
- 新增 `POST /api/profiles` 与 `PUT /api/profiles/{profileId}`，支持账号下新建健康档案和编辑当前档案。
- 前端档案切换已从 `profileType` 改为 `profileId`，避免同类型多档案切换错乱。

仍需继续：

- 饮食图片目前可上传与记录，但还缺真正的食物视觉识别模型和营养结构化。
- 睡眠 7 天趋势仍需要更正式的历史 signals 查询接口和趋势组件。
- 原始图片/PDF/音频的生产级对象存储策略还未接入，目前重点保存元信息和摘要。
- 正式登录仍是邮箱身份登录/创建账号，还不是验证码、密码、微信或 Apple 登录。
- 知识库仍是开发者手动导入的占位卡片，需要补真实中医/健康资料来源。

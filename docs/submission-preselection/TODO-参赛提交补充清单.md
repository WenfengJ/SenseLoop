# anker-SenseLoop 预选提交包与补充清单

这个目录用于准备 Anker 黑客松预选阶段的初始提交材料。本文同时作为提交包索引和后续补充 TODO，避免 README 与清单分散维护。

## 预选材料核心表达

官方预赛材料要求轻量、清晰，核心是赛题理解和解决方案。SenseLoop 的提交口径应聚焦“智能录音赛道 + soundcore Work + 岐黄问诊助手 + 四诊 Agent 工作流”。

当前原型验证的是：

```text
soundcore 夜间声音闻诊入口
  -> 望闻问切四诊输入
  -> 岐黄问诊助手
  -> 中医知识库/RAG 依据
  -> 今日建议、追问、报告和长期记忆
```

预赛材料重点写 2-3 个核心功能，不铺太散：

- 睡眠声音闻诊。
- 岐黄问诊助手。
- 四诊报告和个人健康档案。

## 参赛提交补充清单

这份清单用于记录当前项目距离正式参赛提交还需要补充的内容。  
已排除“当前实现截图”这个已知待补项；产品介绍视频和页面操作介绍视频已准备飞书链接。

## P0：必须补

### 1. 正式 GitHub 仓库链接

目的：

- 让评审可以查看代码结构、README、前后端实现和文档材料。

待完成：

- [x] 确认仓库名。
- [ ] 清理无关文件。
- [x] 检查 `.gitignore`。
- [ ] 提交代码。
- [ ] Push 到 GitHub。
- [x] 将 GitHub 链接填写到 `submission-ready/人工填写项.md`。

### 2. 在线 Demo 链接

目的：

- 让评审不用本地启动，也能直接访问 Demo。

待完成：

- [x] 选择部署方式：自有服务器 Docker + nginx 子路径。
- [x] 部署前端 Demo。
- [x] 验证页面可打开。
- [ ] 验证首页、睡眠页、日报页可切换。
- [x] 将在线 Demo 链接填写到 `submission-ready/人工填写项.md`。

### 3. PPT 或 PDF 答辩材料

目的：

- 让预选材料更像完整比赛项目，而不是只有代码和 Markdown。

建议页数：5-7 页。

建议结构：

- [x] 第 1 页：项目名 + 一句话介绍。
- [x] 第 2 页：真实痛点。
- [x] 第 3 页：解决方案与核心闭环。
- [x] 第 4 页：Demo 展示。
- [x] 第 5 页：技术架构。
- [x] 第 6 页：商业化价值。
- [x] 第 7 页：未来演进与 24 小时现场计划。
- [x] 已生成 PPTX：`docs/submission-preselection/deck/anker-SenseLoop-答辩材料.pptx`。
- [x] 已生成 PDF：`docs/submission-preselection/deck/anker-SenseLoop-答辩材料.pdf`。
- [x] 已生成总览预览图：`docs/submission-preselection/deck/anker-SenseLoop-答辩材料-montage.png`。

### 4. 团队信息

目的：

- 满足官方提交表单要求。

待完成：

- [x] 队伍名称：anker-SenseLoop。
- [x] 成员姓名。
- [x] 成员角色。
- [x] 联系方式：17611650226。
- [x] 是否个人参赛或组队参赛：组队参赛，4 人。
- [x] 填写到 `submission-ready/人工填写项.md`。

### 4.1 飞书预赛文档

目的：

- 按官方模板提交最终文档链接。

待完成：

- [x] 整理可复制到飞书的正式填写稿：`docs/submission-preselection/飞书预赛材料填写稿.md`。
- [ ] 复制到飞书文档。
- [ ] 开启“互联网获取链接可阅读权限”。
- [ ] 将飞书文档链接填入 `submission-ready/人工填写项.md`。
- [ ] 在官方预赛提交问卷中提交飞书文档链接。

### 5. 目标赛道确认

当前建议：

- 主赛道：Smart Audio / soundcore Work 相关方向。
- 辅助扩展：eufy 非隐私视觉观察。

待完成：

- [x] 确认官方提交表单里的赛道名称：智能录音 / Smart Audio。
- [x] 将目标赛道写入 `submission-ready/人工填写项.md`。
- [x] 在 PPT / PDF 中明确写出使用的官方硬件。

## P1：建议补

### 6. 现场讲稿

目的：

- 方便项目组讲解、预选视频录制和现场答辩。

建议新增：

- [ ] 1 分钟电梯稿。
- [ ] 3 分钟路演稿。
- [ ] 5 分钟完整讲稿。
- [ ] 评委问答备忘录。

### 7. 竞品对比表

目的：

- 快速说明 SenseLoop 为什么不是普通睡眠 App，也不是普通智能手表。

建议对比对象：

- [ ] Apple Watch。
- [ ] 华为健康 / 华为手表。
- [ ] 常见睡眠 App。
- [ ] 饮食热量 App。
- [ ] SenseLoop。

建议对比维度：

- [ ] 数据采集。
- [ ] 是否有夜间声音。
- [ ] 是否有饮食/排便/舌苔/口气。
- [ ] 是否生成今日行动建议。
- [ ] 是否结合官方硬件。
- [ ] 是否有未来健康穿戴产品化路径。

### 8. 评委 Q&A

目的：

- 提前准备答辩中的高频问题。

建议问题：

- [x] 为什么不是普通睡眠 App？
- [x] 没有真实 SDK 怎么办？
- [x] 为什么需要 soundcore Work？
- [x] eufy 在这里起什么作用？
- [x] 隐私怎么保证？
- [x] 会不会涉及医疗诊断？
- [x] 24 小时现场还能做什么？
- [x] 未来为什么可以产品化？
- [x] 补充官方赛道、仓库链接、Demo 链接等信息。
- [x] 补充团队成员姓名、角色、是否组队等人工信息。

### 9. README 当前提交状态

目的：

- 让评审快速判断项目当前完成度。

待补充：

- [x] 前端已跑通。
- [x] 后台已预留。
- [x] mock 数据可演示。
- [x] SDK 尚未接入，已通过 adapter 预留。
- [x] API Key 尚未接入，无 API Key 时使用本地规则引擎。
- [x] 现场增强方向已说明。

## P2：可选加分

### 10. Logo / 项目封面图

目的：

- 提高 PPT、README、视频封面的完整度。

待完成：

- [x] 设计 SenseLoop Logo。
- [x] 生成项目封面图。
- [x] 放入 `docs/assets/brand/`。
- [x] 同步放入 `docs/submission-preselection/brand/`。
- [x] 如需提交 PNG，再从 SVG 导出 PNG。

### 11. Demo 二维码

目的：

- 方便现场评委或项目组快速打开 Demo。

待完成：

- [x] 在线 Demo 部署完成。
- [x] 获取公开 Demo 链接。
- [x] 生成二维码。
- [x] 保存到 `docs/submission-preselection/qrcode/demo-qrcode.png`。
- [x] 放入 PPT / PDF。
- [ ] 放入演示视频结尾页或备用页。

### 11.1 系统架构图

目的：

- 让评委快速理解当前原型、技术链路和未来硬件演进。

待完成：

- [x] 生成单图版系统架构图。
- [x] 保存 SVG：`docs/submission-preselection/diagrams/senseloop-system-architecture.svg`。
- [x] 保存 PNG：`docs/submission-preselection/diagrams/senseloop-system-architecture.png`。
- [x] 架构图 GitHub 链接已填写到 `submission-ready/人工填写项.md`。

### 12. 一页式 PDF

目的：

- 让评审快速扫完整项目。

建议内容：

- [ ] 项目名。
- [ ] 一句话介绍。
- [ ] 痛点。
- [ ] 使用硬件。
- [ ] 当前 Demo。
- [ ] 创新点。
- [ ] 技术路线。
- [ ] 商业化价值。

### 13. Roadmap 路线图

目的：

- 展示项目不是一次性 Demo，而是有产品化路径。

建议结构：

- [x] 预选 Demo：mock 数据 + 前端闭环。
- [x] 现场版本：soundcore Work / 上传音频 / 报告增强。
- [x] 可试用 MVP：真实录音 + 7 天趋势 + 用户反馈。
- [x] 产品化版本：SenseLoop Watch / Band / Pendant。
- [x] 已生成 Roadmap 文档：`docs/submission-preselection/Roadmap路线图.md`。

### 14. 百度网盘材料包

目的：

- 如果官方表单需要附件或视频链接，提供一个集中材料包。

待完成：

- [x] 人工上传到百度网盘。
- [x] 填写网盘分享链接到 `submission-ready/人工填写项.md`。
- [x] 确认分享权限可访问，文件名：`anker-SenseLoop-预选提交材料.zip`，提取码：`jvcv`。

建议上传：

- [x] PPT / PDF 答辩材料已准备。
- [x] 系统架构图已准备。
- [x] Demo 二维码已准备。
- [x] 核心截图已准备。
- [x] 产品介绍视频链接已准备。
- [x] 页面操作介绍视频链接已准备。
- [ ] 如官方严格要求 3 分钟以内视频，再按脚本录制或从现有视频剪辑短版。

## 当前优先级建议

建议先按这个顺序推进：

1. GitHub 仓库链接。
2. 在线 Demo 链接。
3. PPT / PDF。
4. 团队信息和目标赛道。
5. 现场讲稿和评委 Q&A。
6. 竞品对比表。
7. Logo、二维码、一页式 PDF、Roadmap。
